...
import com.mycompany.order.config.KafkaProperties;
...
class OrderServiceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/order-service-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/order-service-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
