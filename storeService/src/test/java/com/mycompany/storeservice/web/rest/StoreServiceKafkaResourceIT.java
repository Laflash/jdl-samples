...
import com.mycompany.storeservice.config.KafkaProperties;
...
class StoreServiceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/store-service-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/store-service-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
