...
import com.mycompany.store.web.config.KafkaProperties;
...
class WebStoreKafkaResourceIT {
    ...
    void setup() {
        ...
        WebStoreKafkaResource kafkaResource = new WebStoreKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/web-store-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/web-store-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
